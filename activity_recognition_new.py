...
def do_recognition():
    ...
    today = (date.today() - timedelta(1)).strftime('%Y-%m-%d') #date.today().strftime('%Y-%m-%d')
    yesterday = (date.today() - timedelta(2)).strftime('%Y-%m-%d')
    blob_table = blob_table[(blob_table['date']==today)|(blob_table['date']==yesterday)] 
    ...
    # In[23]:
    watches = list(blob_df['id'].unique())
    print('watches : ', watches)
    for watch_id in [watches[1]]:
        ...
        while (t + whole_window_size < end_time):
            ...
            while(t+window_slide< t_end5min):
                ...
                if snippet_df.shape[0]>= 1:
                    increment +=1
                    ser = ff.extract_features(snippet_df, index=increment, feature_list=feature_list , preserved_features=[])
                    DF = DF.append(ser, verify_integrity=True)
                t = t_end
            #DF.dropna(axis=0, how='any', inplace=True)
            DF.fillna(DF.mean().fillna(0), inplace=True)
            if DF.shape[0]<=11:
                outcome = 7.0
            else:
                X_test = DF.values
